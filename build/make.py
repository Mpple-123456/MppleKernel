...
class BuildUI(tk.Tk):
    def __init__(self):
        ...
        tk.Label(self, text="注意：制作 ISO 需要 grub-common 和 xorriso", fg="gray").pack(pady=2)
    ...
    def compile_kernel(self):
        ...
        cmd = [gxx, "-m32", "-ffreestanding", "-nostdlib", "-fno-builtin",
               "-fno-exceptions", "-fno-rtti", "-O2", "-c", "../kernel/kernel.cpp",
               "-o", "kernel.o", "-std=c++11"]
    ...
    def link_kernel(self):
        ...
        cmd = [ld, "-m", "elf_i386", "-T", "../kernel/linker.ld", "-o", "kernel.elf", "kernel.o"]
        self.run_cmd(cmd)
    ...
    def make_floppy_image(self):
        ...
        r, _ = self.run_cmd([nasm, "-f", "bin", "../boot/boot.asm", "-o", "boot.bin"])
        if not r:
            return
        img_path = os.path.join(self.project_root, "MppleKernel.img")
        try:
...
""")
        ...
        cmd = ["grub-mkrescue", "-o", os.path.join(self.project_root, "MppleKernel.iso"), iso_dir]
        self.run_cmd(cmd, cwd=os.getcwd())
        shutil.rmtree(iso_dir, ignore_errors=True)

    def run_qemu(self):
        qemu = self.entry_qemu.get().strip() or "qemu-system-i386"
        iso_path = os.path.join(self.project_root, "MppleKernel.iso")
        floppy_path = os.path.join(self.project_root, "MppleKernel.img")

        if os.path.exists(iso_path):
            self.run_cmd([qemu, "-cdrom", iso_path, "-snapshot"], capture=False)
        elif os.path.exists(floppy_path):
            self.run_cmd([qemu, "-drive", f"format=raw,file={floppy_path}", "-fda", floppy_path, "-snapshot"], capture=False)
        else:
    ...
    def clean_files(self):
        patterns = ['kernel.o', 'kernel.elf', 'kernel.bin', 'boot.bin', 'iso']
        for p in patterns:
            if os.path.isdir(p):
                shutil.rmtree(p, ignore_errors=True)
                self.log(f"删除目录 {p}", "green")
            else:
                for f in glob.glob(p):
                    try:
                        os.remove(f)
                        self.log(f"删除 {f}", "green")
                    except Exception as e:
                        self.log(f"删除失败 {f}: {e}", "red")
        for img in [os.path.join(self.project_root, "MppleKernel.img"),
                    os.path.join(self.project_root, "MppleKernel.iso")]:
            if os.path.exists(img):
                os.remove(img)
                self.log(f"删除 {img}", "green")
